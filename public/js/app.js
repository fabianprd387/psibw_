...
// Cegah error jika user belum login atau data kosong
if (!userDataString) {
    window.location.href = '/login.html';
} else {
    const userData = JSON.parse(userDataString);

    // Sembunyikan menu jika yang login adalah mahasiswa
    ...
    document.getElementById('menuDashboard').addEventListener('click', function(e) {
                ...
                <div class="col-12">
                    <div class="card shadow-sm">
                        <div class="card-body">
                            ...
                            <p>Anda login sebagai: <strong>${(userData.role || 'User').toUpperCase()}</strong></p>
                        </div>
                    </div>
                </div>
            </div>
            <div class="row" id="laporanCards">
                <div class="col-12 text-center"><p>Memuat data ringkasan...</p></div>
            </div>
        `;
        ...
        fetch('/api/laporan')
            ...
            .catch(() => {
                document.getElementById('laporanCards').innerHTML = '<div class="col-12 text-center text-danger"><p>Gagal memuat ringkasan data.</p></div>';
            });
    });
    ...
    // Jalankan klik dashboard pertama kali
